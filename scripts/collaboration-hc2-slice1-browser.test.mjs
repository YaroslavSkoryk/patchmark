...
const vectorPath = join(repositoryRoot, "scripts", "fixtures", "collaboration-hc2-slice1-v1.json");
const vectors = JSON.parse(readFileSync(vectorPath, "utf8"));
const expected = await createHc2Slice1VectorActual(vectors);
const chromePath = process.env.PATCHMARK_CHROME_PATH ?? findChromeExecutable();
if (!chromePath) throw new Error("Chrome was not found. Set PATCHMARK_CHROME_PATH to run HC-2 vectors.");

const harness = await startHarnessServer();
const profile = mkdtempSync(join(tmpdir(), "patchmark-hc2-slice1-chrome-"));
...
try {
  ...
  assert.equal(outcome.status, "passed", outcome.error ?? "Browser HC-2 vectors failed.");
...
async function waitForOutcome(client) {
  ...
  for (let attempt = 0; attempt < 200; attempt += 1) {
    latest = await evaluate(client, { expression: "globalThis.__patchmarkHc2Slice1Outcome ?? null" });
    if (latest?.status === "passed" || latest?.status === "failed") return latest;
    await delay(50);
  }
  throw new Error(`Timed out waiting for browser HC-2 vectors: ${JSON.stringify(latest)}`);
}
...
function isAllowedTypeScriptPath(pathname) {
  return pathname === "/scripts/collaboration-hc2-slice1-vector-runtime.ts" ||
...
<html lang="en">
  <meta charset="utf-8">
  <title>Patchmark HC-2 Slice 1 vectors</title>
  <body>
    <script type="module">
      globalThis.__patchmarkHc2Slice1Outcome = { status: "running" };
      try {
        const { createHc2Slice1VectorActual } = await import("/scripts/collaboration-hc2-slice1-vector-runtime.ts");
        ...
        const result = await createHc2Slice1VectorActual(await response.json());
        globalThis.__patchmarkHc2Slice1Outcome = { status: "passed", result };
      } catch (error) {
        globalThis.__patchmarkHc2Slice1Outcome = { status: "failed", error: error instanceof Error ? error.stack : String(error) };
      }
    </script>
  </body>
</html>`;
