...
async function openProfile(label, url, existingProfile = null) {
  ...
  try {
    ...
    return { page, profile, product: version.product, async close({ remove = true } = {}) { await boundedCdpClose(page); await boundedCdpClose(browser); process.kill("SIGTERM"); await waitForProcessExit(process, 1200); if (process.exitCode === null) { process.kill("SIGKILL"); await waitForProcessExit(process, 1200); } if (remove) rmSync(profile, { recursive: true, force: true }); } };
